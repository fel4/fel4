...
pub const HEAP_START: usize = 0o_000_100_000_000_0000;
...
impl BumpAllocator {
    /// create a new bump allocator, which uses memory in the
...
/// Align downwards. Returns the greatest x with alignment `align` so
/// that x <= addr. The alignment must be a power of two.
...
/// Align upwards. Returns smallest x with alignment `align` so that
/// x >= addr. The alignment must be a power of two.
...
#[no_mangle]
pub extern fn __rust_usable_size(size: usize, align: usize) -> usize { size }
